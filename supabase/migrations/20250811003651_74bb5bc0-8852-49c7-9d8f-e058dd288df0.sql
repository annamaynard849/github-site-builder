-- Onboarding and Plans tables, plus task personalization columns (retry without IF NOT EXISTS on policies)
...
DROP POLICY IF EXISTS "Users can manage their own onboarding answers" ON public.onboarding_answers;
...
DROP POLICY IF EXISTS "Users can view plan for accessible loved ones" ON public.plans;
CREATE POLICY "Users can view plan for accessible loved ones"
ON public.plans
FOR SELECT
USING (
  EXISTS (
...
DROP POLICY IF EXISTS "Users can upsert plan for accessible loved ones" ON public.plans;
CREATE POLICY "Users can upsert plan for accessible loved ones"
ON public.plans
FOR ALL
USING (
  EXISTS (
